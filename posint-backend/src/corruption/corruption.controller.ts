...
export class CorruptionController {
  ...
  findById(@Param('id') id: string) { return this.corruptionService.findById(id) }
  ...
  update(@Param('id') id: string, @Body() dto: UpdateCaseDto, @CurrentUser() user: JwtPayload) { return this.corruptionService.update(id, dto, user.sub) }

  @Delete('cases/:id') @Roles('ADMIN') @HttpCode(HttpStatus.NO_CONTENT) @ApiBearerAuth() @ApiOperation({ summary: 'Soft-delete case (admin)' })
  remove(@Param('id') id: string, @CurrentUser() user: JwtPayload) { return this.corruptionService.remove(id, user.sub) }
}
